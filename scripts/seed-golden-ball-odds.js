...
// Seed the "Gullballen" (Golden Ball / best player) special market from the
// committed src/data/goldenBallOdds.json snapshot — the single source of truth
...
//
// Run:  SUPABASE_URL=… SUPABASE_SERVICE_ROLE_KEY=… node scripts/seed-golden-ball-odds.js
...
const GOLDEN_BALL_ODDS_PATH = join(__dirname, '..', 'src', 'data', 'goldenBallOdds.json');

async function main() {
  const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY);

  const raw = JSON.parse(readFileSync(GOLDEN_BALL_ODDS_PATH, 'utf8'));
  const bets = raw.available_bets ?? [];
  if (bets.length === 0) {
    console.error('ERROR: goldenBallOdds.json has no available_bets.');
    process.exit(1);
  }
  ...
  // Upsert the 'golden_ball' market and get its id.
  const { data: marketRow, error: marketErr } = await supabase
    .from('special_markets')
    .upsert(
      { key: 'golden_ball', title: 'Beste spiller', status: 'open', locks_at: LOCKS_AT },
      { onConflict: 'key' }
    )
    .select('id')
    .single();
  ...
  if (marketErr) {
    console.error('ERROR upserting golden_ball market:', marketErr.message);
    process.exit(1);
  }
  console.log(`Golden ball market ${marketRow.id} — locks_at=${LOCKS_AT}`);
